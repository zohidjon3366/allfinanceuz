  ...
  if(listRoot){
    ...
    listRoot.innerHTML = news.slice(0, limit).map(item => `
      <article class="news-card reveal">
        <a class="news-cover" href="${document.body.dataset.base || './'}yangilik.html?slug=${item.slug}" style="background-image:linear-gradient(180deg,rgba(17,23,54,.18),rgba(17,23,54,.62)),url(${document.body.dataset.base || './'}${item.cover})"></a>
        <div class="news-body">
  ...
  if(detailRoot){
    ...
    if(item){
      ...
      <div class="news-detail-hero">
        ...
        <div class="detail-cover" style="background-image:linear-gradient(180deg,rgba(17,23,54,.1),rgba(17,23,54,.56)),url(${item.cover})"></div>
      </div></div></section>
