window.AF_NEWS = [
  {
    slug: "soliq-rejalashtirish-2026",
    category: "Soliq",
    date: "2026-07-01",
    readTime: "4 daqiqa",
    title: "2026-yil uchun soliq rejalashtirishda e’tibor beriladigan 7 nuqta",
    excerpt: "Yil boshidan oldin tekshirilishi kerak bo‘lgan hisobot, kontragent, QQS va pul oqimi bo‘yicha amaliy tavsiyalar.",
    cover: "assets/img/bg-news-finance.svg",
    seoDescription: "2026-yil uchun soliq rejalashtirishda asosiy xavflar, hisobotlar va amaliy tavsiyalar.",
    content: [
      "Soliq rejalashtirish faqat deklaratsiyani topshirish bilan tugamaydi. U yil davomida hujjatlar, kontragentlar va operatsion intizomni boshqarishni talab qiladi.",
      "Avvalo, birlamchi hujjatlar aylanishi va kontragentlar bilan hisob-kitoblar o‘z vaqtida yopilib borishi zarur. Bu keyinchalik tafovutlarning ko‘payib ketishini oldini oladi.",
      "QQS to‘lovchilari uchun hisob-fakturalar, import-eksport hujjatlari hamda chegirmalar bazasi alohida ichki nazorat punktlari bilan yuritilishi maqsadga muvofiq.",
      "Oylik boshqaruv hisobotlari rahbar uchun soliq yukini oldindan ko‘rish imkonini beradi. Aynan shu sababli daromad, xarajat va pul oqimi bir formatda yuritilishi kerak."
    ]
  },
  {
    slug: "ichki-audit-amaliy-checklist",
    category: "Audit",
    date: "2026-06-24",
    readTime: "5 daqiqa",
    title: "Ichki audit uchun qisqa checklist: nimadan boshlash kerak?",
    excerpt: "Ichki auditni tez va foydali boshlash uchun 5 bosqichli amaliy tekshiruv ro‘yxati.",
    cover: "assets/img/bg-service-audit.svg",
    seoDescription: "Ichki auditni boshlash uchun amaliy checklist va bosqichma-bosqich yondashuv.",
    content: [
      "Ichki auditning maqsadi kamchilik izlash emas, balki xavf va nazorat nuqtalarini aniqlashdir.",
      "Birinchi bosqichda hisob siyosati, vakolat taqsimoti va hujjatlar aylanish tartibi ko‘rib chiqiladi.",
      "Keyin bank, kassa, kontragentlar, ish haqi va soliq ma’lumotlari tanlab tekshiriladi. Shu bilan birga hujjat va registrlarning o‘zaro bog‘liqligi baholanadi.",
      "Yakunda rahbariyat uchun qisqa, amaliy va ustuvorlikka asoslangan tavsiyalar paketi tayyorlanadi."
    ]
  },
  {
    slug: "1c-avtomatlashtirish-samarasi",
    category: "1C",
    date: "2026-06-10",
    readTime: "3 daqiqa",
    title: "1C xizmatlari orqali buxgalteriya jarayonlarini qanday tezlashtirish mumkin?",
    excerpt: "Hisobot, nomenklatura, bank va cheklar importini soddalashtirish bo‘yicha qisqa tavsiyalar.",
    cover: "assets/img/bg-service-1c.svg",
    seoDescription: "1C avtomatlashtirish yordamida buxgalteriya jarayonlarini tezlashtirish bo‘yicha tavsiyalar.",
    content: [
      "1C xizmatlarining asosiy afzalligi — qo‘l mehnatini kamaytirish va xatolar sonini pasaytirishdir.",
      "Cheklar, bank operatsiyalari va nomenklatura moslashtirish jarayonlari avtomatlashtirilsa, hisobot tayyorlash tezlashadi.",
      "Shuningdek, boshqaruv hisoboti va ichki nazorat uchun zarur kesimlar oldindan sozlab qo‘yilsa, rahbariyatga kerakli ko‘rsatkichlar tezroq chiqariladi."
    ]
  }
];